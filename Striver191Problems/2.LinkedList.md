...
### Missed:
- not getting from where it will stop, so consider k+1 gap b/w two pointers
- maintain dummy nodes for eage case, when we have to delete first/head node it self
- more details [delete-nth-node-from-last](/LinkedList/10.deleteNthNodeFromLast.md)


## 5. Add two numbers represented as Linked Lists
already given digits in reveser order in LL so its easy simple we have to trace
